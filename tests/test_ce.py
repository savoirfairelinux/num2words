...
TEST_CASES_MILLIONS = [
    ...
    (2600260000000000000, "ши триллион ялх бӀе биллиард ши бӀе кхузткъе биллион"),
    (28002800000000000000, "ткъе бархӀ триллион ши биллиард бархӀ бӀе биллион"),
    (300030000000000000000, "кхо бӀе триллион ткъе итт биллиард"),
    (3200320000000000000000, "кхо триллиард ши бӀе триллион кхо бӀе ткъе биллиард"),
    (34003400000000000000000, "ткъе дейтта триллиард кхо триллион ди бӀе биллиард"),
    (360036000000000000000000, "кхо бӀе кхузткъе триллиард ткъе ялхитта триллион"),
    (3800380000000000000000000, "кхо квадриллион бархӀ бӀе триллиард кхо бӀе дезткъе триллион"),
    (40004000000000000000000000, "шовзткъе квадриллион ди триллиард"),
    (420042000000000000000000000, "ди бӀе ткъе квадриллион шовзткъе ши триллиард"),
    (4400440000000000000000000000, "ди квадриллиард ди бӀе квадриллион ди бӀе шовзткъе триллиард"),
    (46004600000000000000000000000, "шовзткъе ялх квадриллиард ди квадриллион ялх бӀе триллиард"),
    (480048000000000000000000000000, "ди бӀе дезткъе квадриллиард шовзткъе бархӀ квадриллион"),
    (5000500000000000000000000000000, "пхи квинтиллион пхи бӀе квадриллион"),
    (52005200000000000000000000000000, "шовзткъе шийтта квинтиллион пхи квадриллиард ши бӀе квадриллион"),
    (540054000000000000000000000000000, "пхи бӀе шовзткъе квинтиллион шовзткъе дейтта квадриллиард"),
    (5600560000000000000000000000000000, "пхи квинтиллиард ялх бӀе квинтиллион пхи бӀе кхузткъе квадриллиард"),
    ]
...
class Num2WordsCETest(TestCase):
    ...
    def test_number(self):
        for test in TEST_CASES_CARDINAL:
            self.assertEqual(num2words(test[0], lang='ce', case=test[1], clazz=test[2]), test[3])
    ...
    def test_ordinal_number(self):
        for test in TEST_CASES_ORDINAL:
            self.assertEqual(num2words(test[0], lang='ce', to="ordinal", clazz=test[2]), test[3])

    def test_currency(self):
        for test in TEST_CURRENCY:
            self.assertEqual(num2words(test[0], lang='ce', to="currency", currency="RUB", case=test[1]), test[2])
